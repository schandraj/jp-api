...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $course = Course::create([
                ...
                'link_record' => $input['link_record'] ?? null,
    ...
    public function index(Request $request)
    {
        try {
            ...
            $query->withCount(['questions', 'topics', 'transactions as student_count' => function ($query) {
                $query->where('status', 'paid');
            }, 'transactions']);

            // Add subquery for lessons count
            $query->addSelect(['videos_count' => function ($query) {
                $query->select(DB::raw('COUNT(topic_lessons.id)'))
                    ->from('topic_lessons')
                    ->join('course_topics', 'topic_lessons.topic_id', '=', 'course_topics.id')
                    ->whereColumn('course_topics.course_id', 'courses.id')
                    ->limit(1);
            }]);
    ...
    {
        try {
            $course = Course::with(['category', 'topics.lessons', 'questions.answers', 'crossSells.crossCourse' => function ($query) {
    ...
    public function update(Request $request, $id)
    {
        ...
        // Decode JSON strings for array fields safely (accept arrays or JSON strings)
        $input = $request->all();
        ...
        foreach ($jsonFields as $field) {
            if ($request->has($field)) {
                ...
                if (is_string($raw)) {
                    ...
                    $input[$field] = $decoded;
                } elseif (is_array($raw) || is_null($raw)) {
                    $input[$field] = $raw; // already an array or explicit null
                } else {
                    return response()->json([$field => "Invalid type for $field"], 422);
                }
            } else {
                // Do not set a default; leaving it unset prevents accidental deletion when not updating this relation
            }
        }
        ...
        try {
            ...
            $course->update([
                ...
                'link_record' => $input['link_record'] ?? $course->link_record,
            ...
            if ($request->has('benefits')) {
                if (!empty($input['benefits'])) {
                    ...
                    // Explicitly detach all benefits when provided as empty array
                    $course->benefits()->sync([]);
                }
            }
    ...
    public function getAnalytics(Request $request, $id)
    {
        try {
            ...
            // Weekly (Last 7 days) transaction count and revenue
            $startDate = Carbon::now()->subDays(6)->startOfDay();
            $weeklyTransactionsLast7Days = Transaction::where('status', 'paid')
                ->where('course_id', $id)
            ...
            foreach ($days as $index => $day) {
                $dayKey = 'day_' . ($index + 1);
                if (isset($weeklyTransactionsLast7Days[$day])) {
                    $weeklyChart[$dayKey] = (int)$weeklyTransactionsLast7Days[$day]->count;
                }
            }

            $weeklyRevenueLast7Days = Transaction::where('status', 'paid')
                ->where('course_id', $id)
                ->where('created_at', '>=', $startDate)
